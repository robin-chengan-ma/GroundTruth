...
2. 測試通過後、提交前，先跑一次靜態檢查：`ruff check .`（涵蓋不到完整依賴鏈、跑不了 `pytest`
   的情況下，至少對本次異動到的檔案跑）。有警告先修，確認乾淨後才摘要本次異動（檔案清單、
...
## 專案覆寫（Project Overrides）— GroundTruth 實際規格

> 產品規格見 `docs/specs/SPEC.md`；本區塊寫死這個專案的技術棧與目錄結構現況，不再是通用範例。
...
| 版控 | GitHub |
| 部署 | Docker Compose（本地一鍵啟動，非公開上線） |

### 技術棧指令對照表

| 動作 | 指令 |
| --- | --- |
| 安裝依賴（backend） | `pip install -r requirements.txt` |
| 安裝依賴（frontend） | `npm install` |
| Lint（backend） | `ruff check .` |
| 單元測試（backend） | `pytest -q` |
| 測試覆蓋率（backend） | `pytest --cov` |
| 本地啟動（全服務） | `docker compose up`（於 `deploy/` 或根目錄，依 Phase 7 實際檔案位置為準） |
| 資料庫 Migration | `python manage.py makemigrations` + `python manage.py migrate` |
| Frontend Lint／測試 | 待 Phase 4 前端開發階段定案工具後補上 |

### 目錄結構慣例

| 資料夾 | 責任 |
| --- | --- |
| `frontend/` | Vue 3 + Vite + Pinia，`src/views`／`stores`／`api`／`router` |
| `backend/` | Django + DRF，`apps/`（各領域 model）＋ `api/`／`services/`／`repositories/`／`schemas/`／`lib/`（分層邏輯） |
| `n8n/workflows/` | n8n workflow 匯出的 JSON，AI 流程協調層 |
| `deploy/` | `docker-compose.yml`、部署腳本 |
| `docs/specs/` | `SPEC.md`（定案）／`PROGRESS.md`（進度）／`DRAFT.md`（未定案） |
| `docs/ADR/discuss/`、`docs/ADR/debug/` | 按功能拆檔的討論與除錯紀錄 |
| `docs/reference/` | API／DB Schema 等技術參考文件 |
| `docs/templates/` | 可攜式母版，供未來新專案複製使用 |

不建立 `mobile/`、`submodules/`：本專案沒有 App；外部服務（LLM、Gmail）串接都由 n8n 節點處理，Django 不需要自建對應的 client 封裝，目前也只有這一個專案在用內部基礎設施程式碼（JWT／API Key 驗證），未達到需要抽成 `submodules/` 共用封裝的程度。不沿用舊專案 `src/` 命名慣例。
