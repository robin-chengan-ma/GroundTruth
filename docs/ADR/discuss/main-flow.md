...
**狀態**：superseded（由 2026-08-28 多角色 RBAC 與獨立簽核政策取代；10,000／100,000 示範門檻沿用）
...
**狀態**：superseded（由 2026-08-28 企業式多品項、多供應商、品質評選與主檔操作取代）
...
## 2026-08-28 [標籤：使用者／AI 提案] 詢價試算與正式提交拆成兩階段

**狀態**：accepted

**背景**：目前「新增詢價」成功後立即建立 Quote 並進入簽核，頁面直接顯示 n8n 回傳的 JSON。
使用者指出「詢價」容易被理解成純查價，且 JSON 是工程師視角，不適合一般企業使用者閱讀；解析結果若
不正確也會在確認前產生正式單號。

**討論內容**：使用者確認方向應改為「先試算、顯示人類可讀結果、確認後才提交」。價格差異也不應只顯示
抽象百分比，應呈現本次單價、歷史平均、價差方向／金額／百分比與是否超過 ±20% 警示門檻。

**決策**：
1. 第一步「試算報價」不建立 Quote，只回傳供應商、產品、數量、單價、總額、歷史平均及價格差異。
2. Vue 使用卡片與狀態文案呈現，不直接顯示 JSON。
3. 第二步「提交採購申請」才建立 Quote、執行幻覺驗證並進入簽核路由。
4. 後端不得信任前端自行回傳的價格或 ID；preview／草稿保存版本與有效期限，正式提交使用 idempotency key，
   並重新驗證版本、價格及報價有效期限。

**理由**：讓一般使用者在正式建單前理解並確認資料，同時維持後端是真實資料與金額計算的唯一可信來源。

**後果**：需調整 Django quote calculation API/service、n8n workflow、Vue InquiryView、測試、API Reference，
可能新增 preview 資料模型與 migration；定案前不得直接實作。

## 2026-08-28 [標籤：使用者／AI 提案] 企業式多品項、多供應商、品質評選與主檔操作

**狀態**：accepted

**背景**：使用者指出，目前一筆 Quote 只能承接一間供應商、一個品項，且前端尚未提供供應商、品項、
供應商價格與庫存的正式操作頁面。若只補價格比價，仍無法反映企業採購對產品規格、品質、交期、保固、
供應商履約與到貨驗收的要求，也容易在面試展示時被追問資料來源、維護責任及選商依據。

**討論內容**：
1. 同一採購需求應包含多個品項；同一 RFQ 可邀請多間供應商，各供應商各自提出一份報價，不把不同
   供應商混在同一張 Quote。
2. 自然語言對話框作為主要輸入與修改入口，可追加／移除品項、修改數量、增減候選供應商及表達選商方式；
   LLM 只產生候選結構，後端固定規則負責資料驗證、金額計算與狀態控制，每次變更後都要用結構化畫面讓
   使用者確認。
3. 供應商、品項、供應商價格、庫存與採購建議頁面都需要受權限控制的操作功能，不是唯讀展示頁；正式
   主檔不採任意實體刪除，而以啟用／停用、版本、原因與稽核紀錄保存歷史一致性。
4. 品項需保存尺寸、材質、顏色、特色、承重、保固、認證及驗收標準等規格；每次採購需求另分必要條件、
   偏好條件與一般描述，避免把共用品項主檔和單次採購要求混在一起。
5. 比價應升級為綜合評選：先檢查必要規格，未通過者不得因低價直接獲推薦；通過後再比較價格、品質／規格、
   交期、保固及供應商履約。系統只提出建議，最後得標決定仍由企業人員確認。
6. 品質分成採購前與採購後：採購前依供應商規格、樣品／認證、保固及歷史履約評估；到貨後記錄收貨、
   合格／瑕疵數量、瑕疵類型、退貨與驗收結果，回饋後續供應商評分。
7. 供應商可提供資料與報價，但不得直接修改企業正式主檔；本專案先以企業內部維護及模擬供應商報價實作，
   外部供應商 Portal、Email Parser 或 API 串接保留為未來擴充點。

**已確認方向**：
1. 正式範圍包含多品項、多供應商 RFQ、供應商獨立報價、比價選商、規格品質評選與到貨驗收回饋。
2. 補齊供應商清單、品項清單／詳情、供應商價格、庫存、採購建議、詢價比價、驗收及稽核等可操作頁面。
3. 詢價維持「對話輸入＋結構化確認＋明確提交」模式，不讓 LLM 直接建立或修改正式單據。

**決策**：
1. 核心單據分為 Purchase Request、RFQ、Supplier Quote、Award、Approval、Purchase Order、Goods Receipt／
   Quality Inspection；各自維護獨立狀態機，不再由單一 Quote 混合承擔所有語意。
2. 得標以逐項 `award_lines` 建模，資料層支援整單、逐品項及同品項拆量；第一版 UI 以整單選商與各品項
   選最低合格報價為主要操作，進階操作可逐項調整。
3. 評選先檢查必要條件，再以案件快照權重計算實際總成本、規格品質、交期、保固與歷史履約；必要條件
   例外及非推薦者得標必須填理由並留下額外核准。AI 只解釋，不計分或決定得標。
4. 簽核核准後只產生採購單；供應商交貨並完成品質驗收後，只有合格數量可以寫入不可覆寫的庫存流水。
5. 權限改為多角色 RBAC 與職責分離；申請人不得核准自己的案件，系統管理員不自動取得業務決議權，
   簽核依得標後實際總成本與獨立 `approval_policies` 路由；10,000／100,000 維持為目前 Demo 預設門檻，
   但不再存於角色本身。
6. 正式單據保存規格、價格與商務條件快照；報價使用版本與有效期限，草稿採版本控制，提交採 idempotency
   key 並由後端重新驗證。
7. 舊 Quote 使用新表並行、可逆資料遷移及 `legacy_quote_id`；新 API 切換後舊表先保留唯讀，Phase 4.1
   不執行 DROP。舊 approved Quote 轉成遷移用已收貨／驗收紀錄，但不得再次增加既有庫存。
8. 核心資料表與狀態轉移細節依本次決策製作 ERD、Migration SQL 與狀態矩陣；任何 Migration 執行前仍須
   依 AGENTS.md 另行提出影響、資料轉換、索引／鎖表風險及回滾方案並取得使用者核准。
9. 功能增加後，主要導覽改採左側單欄垂直階層：大分類與子功能由上而下排列，子功能縮排並可隨分類收合，
   不採多個大分類橫向並排；側欄超出視窗時獨立捲動，窄螢幕改為抽屜式導覽，並依 RBAC 隱藏無權限項目。

**理由**：企業採購的核心不是單純選最低價，而是以可信的主檔、規格符合度、商務條件、品質與履約紀錄支持
可追溯的人工作業與決策。採用對話輸入但以固定規則驗證及人工確認，可同時展示 AI 能力與企業信任邊界。
