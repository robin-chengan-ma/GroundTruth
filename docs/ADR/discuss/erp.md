...
**狀態**：superseded（由 2026-08-28「收貨驗收與庫存流水」決策取代）
...
2. ~~權限限管理員角色操作，一般員工/簽核人看不到此操作。~~ **【superseded，2026-08-28】** 改由 `purchase_suggestion.dismiss` 權限控制，見下方「收貨驗收與庫存流水」決策。
...
## 2026-08-28 [標籤：使用者／AI 提案] 收貨驗收與庫存流水取代核准即入庫

**狀態**：accepted

**背景**：企業採購核心升級加入正式採購單、部分交貨與品質驗收後，舊有「簽核核准即增加庫存」會把
尚未到貨、短交或瑕疵品誤算為可用庫存，必須由可追溯的收貨與驗收流程取代。

**討論內容**：庫存正確性應由不可覆寫的異動流水保證；目前餘額只作查詢快照。採購建議仍採事件驅動，
但觸發來源改為驗收合格入庫、出庫、退貨或人工調整等正式庫存異動。

**決策**：
1. 簽核核准只產生採購單，不增加庫存；供應商交貨後建立收貨及品質驗收，只有合格數量入庫。
2. `inventory_movements` 為庫存真相來源，`inventory_balances` 為查詢快照；錯誤以反向更正流水處理，
   不直接覆寫或刪除歷史異動。
3. 採購單支援部分交貨；收貨需保存實收、合格、瑕疵、拒收、退貨與補交數量。
4. 庫存餘額異動後即時檢查門檻並產生採購建議，不改用排程輪詢。
5. 採購建議可轉成需求草稿並保留來源；忽略操作改由 `purchase_suggestion.dismiss` 權限控制，不綁定
   系統管理員角色。

**理由**：把「核准」、「到貨」與「品質合格」分離，才能在部分交貨、瑕疵、退貨與補交情境維持庫存正確；
流水帳亦能支援稽核與未來 ERP 串接，而不需再次推翻庫存模型。

**後果**：既有 `inventory.stock_qty` 與核准即入庫邏輯成為 legacy；Phase 4.1 需新增收貨、驗收、庫存流水／
餘額模型及可逆資料遷移。舊 approved Quote 的歷史入庫只建立遷移標記，不得重複增加現有庫存。
