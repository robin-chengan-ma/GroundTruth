...
| 2026-08-31 | Phase 5.0／FR-1a／FR-2／FR-6a／FR-16～FR-19／NFR-1 | 應用切換與安全收斂 | Codex／Robin | P5.0-A、P5.0-B1／B2／B3A 與 P5.0-C1 完成；P5.0-C2～E 尚未開發 | 新版候選解析改為 Django 先遮罩多間已知供應商與金額、n8n 回傳後於單次請求遞迴還原；未知、混合已知／未知及模糊供應商不得把原始名稱送至 n8n。Robin 已完成真實 n8n／瀏覽器驗收。本人需求分頁、唯讀詳情、legacy inquiry／Quote command 封鎖及新版簽核工作台已完成；下一步依 `docs/ADR/discuss/phase5.md` 執行 P5.0-C2～E。各切片測試與驗收證據見下方獨立紀錄 |
...
| 2026-09-01 | P5.0-C1／FR-8a／FR-9／FR-9a／FR-16b | 新版簽核工作台與 legacy 簽核命令退役 | Codex／Robin | 程式與自動測試完成；待 Robin 瀏覽器驗收 | 簽核工作台改讀 `ApprovalCase`／`ApprovalStep`，顯示申請編號、用途、申請人、政策、總額、案件狀態及各步驟順序、類型、角色、認領人；認領與決策改呼叫 step action，決策理由必填。後端依角色與權限回傳逐步 `can_claim`／`can_decide`，前端不再自行推測。legacy `Approval` GET 保留歷史唯讀，claim／decide 通過認證後統一回 410／`legacy_command_retired`。TDD RED：Backend 3 failed、Frontend 2 failed；GREEN 目標 Backend 18 passed、Frontend 2 passed；完整 Backend 343 passed、Frontend 25 passed，Ruff、Django check、Migration check、type-check、ESLint、production build、`git diff --check` 通過。不新增 Migration、不修改 n8n |
| 2026-09-01 | Phase 5～7／FR-1～FR-19／NFR-1～NFR-4 | 剩餘 Roadmap 收斂 | Robin／Codex | 已定案；Phase 5 尚未完成 | 後續只使用三個正式階段，不再拆成 Phase 5.0 或 A／B／C 子階段：Phase 5 完成後端新核心切換、安全與完整 API 契約；Phase 6 一次完成全部企業採購操作介面與各角色瀏覽器流程；Phase 7 完成 n8n、Gmail、Docker Compose、Demo 資料、全系統 E2E、文件與發布稽核。既有 P5.0-A／B／C 紀錄保留為已完成工作的歷史證據並併入 Phase 5 驗收，不代表 Phase 5 已完成；每個階段只有全部範圍與完成門檻通過後才可標記完成，不使用部分完成宣稱。決策見 `docs/ADR/discuss/phase5.md` |
